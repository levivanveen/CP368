...
int main(int argc, char *argv[]) {
    ...
    sem_init(&even, 0, evenInit);
    sem_init(&odd, 0, oddInit);
...
//your synchronization logic will appear here
    ...
    sem_getvalue(&even, &evenInt);
    sem_getvalue(&odd, &oddInt);

    if (((Thread*) t)->dontWait == 0) {
        if ((id % 2) == 0) {
            // id is even
            sem_wait(&even);
        } else {
